...
/// Linear-interpolation resampler from `source_rate` down-/upsampled mono
/// samples to [`WHISPER_SAMPLE_RATE`]. Public so hosts can reuse it for audio
/// that did not come from a [`Dictation`] session (e.g. uploaded files).
pub fn resample_mono(input: &[f32], source_rate: u32) -> Vec<f32> {
